    ...
    while (arquivo >> tipo) {
      ...
      case 'F':
        ...
        estoque.Adiciona_filme(
            new Fita(codigo, titulo, quantidade, rebobinada));
        break;
      case 'D':
        arquivo >> quantidade >> codigo >> titulo >> categoria;
        if (categoria == "Lancamento") {
          estoque.Adiciona_filme(
              new DVD(codigo, titulo, quantidade, DVD::Lancamento));
        } else if (categoria == "Estoque") {
          estoque.Adiciona_filme(
              new DVD(codigo, titulo, quantidade, DVD::Estoque));
        } else if (categoria == "Promocao") {
          estoque.Adiciona_filme(
              new DVD(codigo, titulo, quantidade, DVD::Promocao));
        } else {
